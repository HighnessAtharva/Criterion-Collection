...
class CollectionScraperSpider(scrapy.Spider):
    ...
    start_urls = [
        "https://www.criterion.com/shop/browse/list?sort=spine_number&direction=desc"
    ]
    ...
    def parse(self, response):
        ...
        for movie in movies:
            ...
            criterion_movie = CriterionMovieLoader(
                item=CriterionMovieItem(), selector=movie
            )
    ...
    def parse_movie_page(self, response):
        criterion_movie = CriterionMovieLoader(
            item=CriterionMovieItem(), response=response
        )
        criterion_movie.add_value("page_url", response.url)
        criterion_movie.add_value("spine", response.meta["criterion_movie"]["spine"])
        criterion_movie.add_value(
            "thumb_url", response.meta["criterion_movie"]["thumb_url"]
        )
        criterion_movie.add_value("movie", response.meta["criterion_movie"]["movie"])
        criterion_movie.add_value(
            "director", response.meta["criterion_movie"]["director"]
        )
        criterion_movie.add_value(
            "country", response.meta["criterion_movie"]["country"]
        )
        ...
        if poster_url := response.xpath(
            "//div[@class='product-box-art']/img/@src"
        ).get():
            criterion_movie.add_value("poster_url", poster_url)

        if runtime := response.xpath(
            "//li/meta[@itemprop='duration']/following-sibling::text()"
        ).get():
            criterion_movie.add_value("runtime", runtime.replace(" minutes", ""))

        if isColor := response.xpath(
            "//li[meta[@itemprop='duration']]/following-sibling::li[1]/text()"
        ).get():
            criterion_movie.add_value("isColor", isColor)

        if aspect_ratio := response.xpath(
            "//li[meta[@itemprop='duration']]/following-sibling::li[2]/text()"
        ).get():
            criterion_movie.add_value("aspect_ratio", aspect_ratio)

        if language := response.xpath(
            "//li[@itemprop='inLanguage']/span[@itemprop='name']/text()"
        ).get():
        ...
        for f in formats:
            format_name = f.xpath(
                "./label/span[@class='meta-item']/span[@class='item']/text()"
            ).get()
            format_price = (
                f.xpath(
                    "./label/span[@class='meta-prices']/span[@class='item-price']/text()"
                )
                .get()
                .replace("$", "")
            )
